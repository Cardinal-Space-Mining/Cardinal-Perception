    ...
    trajectory_filter{}
{
    ...
    this->scan_sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        scan_topic, rclcpp::SensorDataQoS{},  [this](const sensor_msgs::msg::PointCloud2::ConstSharedPtr& ptr){this->scan_callback(ptr);}, ops);
    this->imu_sub = this->create_subscription<sensor_msgs::msg::Imu>(
        imu_topic, rclcpp::SensorDataQoS{}, [this](const sensor_msgs::msg::Imu::SharedPtr ptr){this->imu_callback(ptr);}, ops);
    ...
    const std::string& info_topic)
{
    ...
    this->image_sub = this->pnode->img_transport.subscribe( img_topic, rmw_qos_profile_sensor_data,
        [this](const image_transport::ImageTransport::ImageConstPtr & ptr){this->img_callback(ptr);},
    ...
    this->info_sub = this->pnode->create_subscription<sensor_msgs::msg::CameraInfo>( info_topic, rclcpp::SensorDataQoS{},
        [this](const image_transport::ImageTransport::CameraInfoConstPtr & ptr){this->info_callback(ptr);}, ops );
}
    ...
    if(dlo_status)
    {
        ...
        if(!(dlo_status & (1 << 1)))
        {
            ...
            if(stable)
            {
                ...
                // odom_match << keypose.second.odometry;
