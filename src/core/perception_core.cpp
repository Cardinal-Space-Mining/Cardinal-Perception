...
void PerceptionNode::getParams()
{
    ...
    util::declare_param(this, "tag_usage_mode", this->param.tag_usage_mode, -1); )
...
void PerceptionNode::initPubSubs()
{
    ...
    this->path_plan_service =
        this->create_service<UpdatePathPlanSrv>(
            ...
            [this](UpdatePathPlanSrv::Request::SharedPtr req,
    ...
    this->velocity_pub =
        this->create_publisher<TwistStampedMsg>(
            PERCEPTION_TOPIC("odom_velocity"),
            PERCEPTION_PUBSUB_QOS );

    this->proc_metrics_pub =
        this->create_publisher<ProcessMetricsMsg>(
...
void PerceptionNode::handleStatusUpdate()
{
    ...
    const auto& proc_stats_map = this->metrics.manager.readProcStatistics(lock_);
    for(const auto& proc_stat : proc_stats_map)
    {
        const char* proc_string = PROC_STRINGS[static_cast<size_t>(proc_stat.first)];
        if(!*proc_string) continue;
    ...
    size_t idx = 0;
    auto& thread_durations_map = this->metrics.manager.getThreadDurations(lock_);
    for(auto& thread_durs : thread_durations_map)
    {
        ...
        for(auto& proc_dur : thread_durs.second)
        {
            ...
            if(n_chars > 0)
            {
                ...
                msg << PROC_COLORS[x] << std::setfill('=') << std::setw(n_chars) << PROC_CHARS[x];
            }
        }
        msg << "\033[0m";
        if(avail_chars > 0)
