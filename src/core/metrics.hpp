...
template<
    typename Key_T,
    typename Clock_T = std::chrono::system_clock>
...
public:
    ...
    using ThreadDurationsMap = std::unordered_map<
                                    std::thread::id,
                                    ProcDurationsMap >;
    using StatisticsMap = std::unordered_map<
                                KeyT,
                                util::proc::ThreadMetrics >;

    using KeyArgT = typename std::conditional<
...
MetricsManager<K, C>::getProcDurations(const KeyArgT key)
{
    ...
    {
        auto x = durs.insert( {key, {0., TimePoint::min()}} );
        if(!x.second) return nullptr;
        else times_ptr = x.first;
    }
...
template<typename K, typename C>
typename MetricsManager<K, C>::TimePoint
MetricsManager<K, C>::registerProcEnd(const KeyArgT key, bool track_stats)
{
    std::lock_guard<std::mutex> lock_{ this->mtx };
    auto tp = ClockT::now();
    ...
    if(times->second > TimePoint::min())
    {
        ...
        times->first += std::chrono::duration<double>(tp - times->second).count();
