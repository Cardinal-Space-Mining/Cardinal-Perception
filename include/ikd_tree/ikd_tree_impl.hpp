...
IKDTree<PointType>::IKDTree(float delete_param, float balance_param, float box_length)
{
...
template <typename PointType>
void IKDTree<PointType>::InitializeKDTree(float delete_param, float balance_param, float box_length)
{
...
template <typename PointType>
void IKDTree<PointType>::InitTreeNode(KD_TREE_NODE *root)
{
    root->point.x = 0.0f;
    root->point.y = 0.0f;
    root->point.z = 0.0f;
...
template <typename PointType>
BoxPointType IKDTree<PointType>::tree_range()
{
    BoxPointType range;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
    {
...
template <typename PointType>
int IKDTree<PointType>::validnum()
{
    int s = 0;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
    {
...
template <typename PointType>
void IKDTree<PointType>::run_operation(KD_TREE_NODE **root, Operation_Logger_Type operation)
{
    switch (operation.op)
    {
    case ADD_POINT:
...
template <typename PointType>
void IKDTree<PointType>::Nearest_Search(PointType point, int k_nearest, PointVector &Nearest_Points, vector<float> &Point_Distance, float max_dist)
{
...
template <typename PointType>
void IKDTree<PointType>::Box_Search(const BoxPointType &Box_of_Point, PointVector &Storage)
{
    Storage.clear();
...
template <typename PointType>
void IKDTree<PointType>::Radius_Search(PointType point, const float radius, PointVector &Storage)
{
    Storage.clear();
...
template <typename PointType>
int IKDTree<PointType>::Add_Point(PointType PointToAdd, bool downsample_on)
{
    // std::cout << "IKD ADD POINT" << std::endl;
    ...
    int tmp_counter = 0;
    {
        if (downsample_switch)
        {
            ...
            Box_of_Point.vertex_min[0] = floor(PointToAdd.x / downsample_size) * downsample_size;
            Box_of_Point.vertex_max[0] = Box_of_Point.vertex_min[0] + downsample_size;
            Box_of_Point.vertex_min[1] = floor(PointToAdd.y / downsample_size) * downsample_size;
            Box_of_Point.vertex_max[1] = Box_of_Point.vertex_min[1] + downsample_size;
            Box_of_Point.vertex_min[2] = floor(PointToAdd.z / downsample_size) * downsample_size;
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
            {
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd, downsample_result))
                {
        ...
        else
        {
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
            {
                ...
                Add_by_point(&Root_Node, PointToAdd, true, Root_Node->division_axis);   // TODO: segfault here
            }
            else
            {
                // std::cout << "\tEXHIBIT H" << std::endl;
                Operation_Logger_Type operation;
                operation.point = PointToAdd;
                operation.op = ADD_POINT;
                pthread_mutex_lock(&working_flag_mutex);
                Add_by_point(&Root_Node, PointToAdd, false, Root_Node->division_axis);
                if (rebuild_flag)
                {
                    pthread_mutex_lock(&rebuild_logger_mutex_lock);
                    Rebuild_Logger.push(operation);
...
template <typename PointType>
void IKDTree<PointType>::BuildTree(KD_TREE_NODE **root, int l, int r, PointVector &Storage)
{
    if (l > r)
        return;
...
template <typename PointType>
void IKDTree<PointType>::Rebuild(KD_TREE_NODE **root)
{
    KD_TREE_NODE *father_ptr;
...
template <typename PointType>
int IKDTree<PointType>::Delete_by_range(KD_TREE_NODE **root, BoxPointType boxpoint, bool allow_rebuild, bool is_downsample)
{
...
template <typename PointType>
void IKDTree<PointType>::Delete_by_point(KD_TREE_NODE **root, PointType point, bool allow_rebuild)
{
    if ((*root) == nullptr || (*root)->tree_deleted)
        return;
    (*root)->working_flag = true;
...
template <typename PointType>
void IKDTree<PointType>::Add_by_range(KD_TREE_NODE **root, BoxPointType boxpoint, bool allow_rebuild)
{
    if ((*root) == nullptr)
        return;
    (*root)->working_flag = true;
...
template <typename PointType>
void IKDTree<PointType>::Add_by_point(KD_TREE_NODE **root, PointType point, bool allow_rebuild, int father_axis)
{
    if (*root == nullptr)
    {
        *root = new KD_TREE_NODE;
...
template <typename PointType>
void IKDTree<PointType>::Search(KD_TREE_NODE *root, int k_nearest, PointType point, MANUAL_HEAP &q, float max_dist)
{
    if (root == nullptr || root->tree_deleted)
        return;
...
template <typename PointType>
void IKDTree<PointType>::Search_by_range(KD_TREE_NODE *root, BoxPointType boxpoint, PointVector &Storage)
{
    if (root == nullptr)
        return;
    Push_Down(root);
...
template <typename PointType>
void IKDTree<PointType>::Search_by_radius(KD_TREE_NODE *root, PointType point, float radius, PointVector &Storage)
{
    if (root == nullptr)
        return;
    Push_Down(root);
...
template <typename PointType>
bool IKDTree<PointType>::Criterion_Check(KD_TREE_NODE *root)
{
...
template <typename PointType>
void IKDTree<PointType>::Push_Down(KD_TREE_NODE *root)
{
    if (root == nullptr)
        return;
    Operation_Logger_Type operation;
...
template <typename PointType>
void IKDTree<PointType>::Update(KD_TREE_NODE *root)
{
...
template <typename PointType>
void IKDTree<PointType>::flatten(KD_TREE_NODE *root, PointVector &Storage, delete_point_storage_set storage_type)
{
    if (root == nullptr)
        return;
    Push_Down(root);
...
template <typename PointType>
void IKDTree<PointType>::delete_tree_nodes(KD_TREE_NODE **root)
{
    if (*root == nullptr)
        return;
    Push_Down(*root);
...
template <typename PointType>
float IKDTree<PointType>::calc_box_dist(KD_TREE_NODE *node, PointType point)
{
    if (node == nullptr)
        return INFINITY;
    float min_dist = 0.0;
...
template <typename PointType>
bool IKDTree<PointType>::point_cmp_x(PointType a, PointType b) { return a.x < b.x; }
template <typename PointType>
bool IKDTree<PointType>::point_cmp_y(PointType a, PointType b) { return a.y < b.y; }
template <typename PointType>
bool IKDTree<PointType>::point_cmp_z(PointType a, PointType b) { return a.z < b.z; }
